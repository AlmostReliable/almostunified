...
public final class TagConfig extends Config {
    ...
    private final Map<Identifier, Set<Identifier>> customTags;
    private final Map<Identifier, Set<Identifier>> tagSubstitutions;
    ...
    private TagConfig(
        Map<Identifier, Set<Identifier>> customTags, Map<Identifier, Set<Identifier>> tagSubstitutions,
        TagInheritance.Mode itemTagInheritanceMode, Map<TagKey<Item>, Set<Pattern>> itemTagInheritance,
        TagInheritance.Mode blockTagInheritanceMode, Map<TagKey<Block>, Set<Pattern>> blockTagInheritance, boolean emiStrictHiding
    ) {
        super(NAME);
    ...
    public Map<Identifier, Set<Identifier>> getCustomTags() {
        return Collections.unmodifiableMap(customTags);
    }

    public Map<Identifier, Set<Identifier>> getTagSubstitutions() {
    ...
    public static final class TagSerializer extends Config.Serializer<TagConfig> {
        ...
        public TagConfig handleDeserialization(JsonObject json) {
            Map<Identifier, Set<Identifier>> customTags = safeGet(
                () -> JsonUtils.deserializeMapSet(
                    json,
                    CUSTOM_TAGS,
                    e -> Identifier.parse(e.getKey()),
                    Identifier::parse
                ), new HashMap<>()
            );

            Map<Identifier, Set<Identifier>> tagSubstitutions = safeGet(
                () -> JsonUtils.deserializeMapSet(
                    json,
                    TAG_SUBSTITUTIONS,
                    e -> Identifier.parse(e.getKey()),
            ...
            TagInheritance.Mode itemTagInheritanceMode = deserializeTagInheritanceMode(
                json,
                ITEM_TAG_INHERITANCE_MODE
            );
            Map<TagKey<Item>, Set<Pattern>> itemTagInheritance = deserializePatternsForLocations(
                Registries.ITEM,
                json,
                ITEM_TAG_INHERITANCE
            );
            TagInheritance.Mode blockTagInheritanceMode = deserializeTagInheritanceMode(
                json,
                BLOCK_TAG_INHERITANCE_MODE
            );
            Map<TagKey<Block>, Set<Pattern>> blockTagInheritance = deserializePatternsForLocations(
                Registries.BLOCK,
                json,
        ...
        public JsonObject serialize(TagConfig config) {
            ...
            config.customTags.forEach((parent, child) -> {
                customTags.add(
                    parent.toString(),
                    JsonUtils.toArray(child.stream().map(Identifier::toString).toList())
                );
            });
         ...
         */
        private <T> Map<TagKey<T>, Set<Pattern>> unsafeDeserializePatternsForLocations(
            ResourceKey<Registry<T>> registry, JsonObject rawConfigJson, String baseKey) {
            return JsonUtils.deserializeMapSet(
                rawConfigJson,
                baseKey,
                e -> TagKey.create(registry, Identifier.parse(e.getKey())),
                Pattern::compile
            );
        }

        private <T> Map<TagKey<T>, Set<Pattern>> deserializePatternsForLocations(
            ResourceKey<Registry<T>> registry, JsonObject rawConfigJson, String baseKey) {
            return safeGet(
                () -> unsafeDeserializePatternsForLocations(registry, rawConfigJson, baseKey),
                new HashMap<>()
            );
        }
        ...
        private TagInheritance.Mode deserializeTagInheritanceMode(JsonObject json, String key) {
            return safeGet(
                () -> TagInheritance.Mode.valueOf(json
