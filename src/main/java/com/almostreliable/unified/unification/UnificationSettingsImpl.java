...
public final class UnificationSettingsImpl implements UnificationSettings {
    ...
    private final Function<Identifier, Boolean> recipeTypeCheck;
    private final Function<Identifier, Boolean> recipeIdCheck;
    private final boolean recipeViewerHiding;
    private final boolean lootUnification;
    private final Function<Identifier, Boolean> lootTableCheck;
    private final UnificationLookup unificationLookup;
    private final Runnable clearCaches;

    private UnificationSettingsImpl(
        String name, ModPriorities modPriorities, StoneVariants stoneVariants, Function<Identifier, Boolean> recipeTypeCheck,
        Function<Identifier, Boolean> recipeIdCheck, boolean recipeViewerHiding, boolean lootUnification,
        Function<Identifier, Boolean> lootTableCheck, UnificationLookup unificationLookup, Runnable clearCaches
    ) {
        this.name = name;
    ...
    public static List<UnificationSettings> create(
        Collection<UnificationConfig> configs, VanillaTagWrapper<Item> itemTags, VanillaTagWrapper<Block> blockTags,
        TagSubstitutionsImpl tagSubstitutions
    ) {
        return configs
            .stream()
    ...
    public static UnificationSettings create(
        UnificationConfig config, VanillaTagWrapper<Item> itemTags, VanillaTagWrapper<Block> blockTags, TagSubstitutions tagSubstitutions) {
    ...
    @Override
    public boolean shouldIncludeRecipeType(Identifier type) {
        return recipeTypeCheck.apply(type);
    }

    @Override
    public boolean shouldIncludeRecipeId(Identifier id) {
    ...
    @Override
    public boolean shouldIncludeLootTable(Identifier table) {
    ...
    @Nullable
    @Override
    public UnificationEntry<Item> getItemEntry(Identifier item) {
        return unificationLookup.getItemEntry(item);
    }

    @Nullable
    @Override
    public TagKey<Item> getRelevantItemTag(Identifier item) {
        return unificationLookup.getRelevantItemTag(item);
    }

    @Nullable
    @Override
    public UnificationEntry<Item> getVariantItemTarget(Identifier item) {
        return unificationLookup.getVariantItemTarget(item);
    }

    @Nullable
    @Override
    public UnificationEntry<Item> getTagTargetItem(TagKey<Item> tag, Predicate<Identifier> itemFilter) {
