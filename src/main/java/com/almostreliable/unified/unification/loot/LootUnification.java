...
public final class LootUnification {
    ...
    public static void unifyLoot(AlmostUnifiedRuntime runtime, HolderLookup.Provider registries) {
        try {
            ...
            lootTableRegistry
                .listElements()
                .forEach(holder -> unifyLoot(holder.value(), holder.key().identifier(), handlers));
        } catch (Exception e) {
            AlmostUnifiedCommon.LOGGER.error("Failed to unify loot", e);
        }
    }

    public static void unifyLoot(LootTable lootTable, Identifier tableId, Collection<? extends UnificationSettings> unificationSettings) {
