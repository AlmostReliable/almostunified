...
public class VanillaTagWrapper<T> {
    ...
    private final Map<Identifier, Collection<Holder<T>>> vanillaTags;
    @Nullable
    private Map<Holder<T>, Set<Identifier>> holdersToTags;
    private final Set<Identifier> modifiedTags = new HashSet<>();

    public static <T> VanillaTagWrapper<T> of(Registry<T> registry, Map<Identifier, Collection<Holder<T>>> vanillaTags) {
        return new VanillaTagWrapper<>(registry, vanillaTags);
    }

    public VanillaTagWrapper(Registry<T> registry, Map<Identifier, Collection<Holder<T>>> vanillaTags) {
        this.registry = registry;
        this.vanillaTags = vanillaTags;
    }

    public void add(Identifier tag, Holder<T> holder) {
    ...
    public Collection<Holder<T>> get(Identifier tag) {
        return Collections.unmodifiableCollection(vanillaTags.getOrDefault(tag, Collections.emptyList()));
    }

    public Set<Identifier> getTags(Identifier entryId) {
        var key = ResourceKey.create(registry.key(), entryId);
        return registry.get(key).or(registry::getAny).map(this::getTags).orElseGet(Set::of);
    }

    public Set<Identifier> getTags(UnificationEntry<T> entry) {
        return getTags(entry.asHolderOrThrow());
    }

    public Set<Identifier> getTags(Holder<T> holder) {
    ...
    private Map<Holder<T>, Set<Identifier>> createInvertMap() {
        Map<Holder<T>, Set<Identifier>> map = new HashMap<>();
    ...
    public void seal() {
        for (Identifier modifiedTag : modifiedTags) {
