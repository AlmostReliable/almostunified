...
public class ModConfig {
    ...
    public ModConfig(String name) {
        config = CommentedFileConfig.ofConcurrent(UniTaggedPlatform.INSTANCE.getConfigPath().resolve(name + ".toml"));
        spec = new ConfigSpec();
    ...
    private static List<String> getDefaultPatterns() {
        if (UniTaggedPlatform.INSTANCE.getPlatformName().equals("Forge")) {
    ...
    public void load() {
        ...
        if (!spec.isCorrect(config)) {
            UniTagged.LOG.warn("Config has missing or invalid values - correcting now");
            spec.correct(config);
            config.save();
        }
        ...
        for (String pattern : patterns) {
            ...
            if (pattern.startsWith("!")) {
                ...
                for (String s : compiledPattern) {
                    ...
                    if (rl == null) {
                        UniTagged.LOG.warn("Invalid pattern: " + s);
                    } else {
    ...
    private Collection<String> compilePattern(String pattern, Multimap<String, String> variables) {
        ...
        while (!stack.isEmpty()) {
            ...
            if (firstBracket > secondBracket) {
                UniTagged.LOG.warn("Invalid pattern: {}, will be skipped", p);
                return new HashSet<>();
            }

            String toReplace = p.substring(firstBracket + 1, secondBracket);
            if (!variables.containsKey(toReplace)) {
                UniTagged.LOG.warn("Variable {} is not defined in config, pattern will be skipped", toReplace);
                return new HashSet<>();
            }
